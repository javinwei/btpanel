...
import common,public,data,page,db,time,json
...
web.config.session_parameters['secret_key'] = 'www.baidu.com'
...
class panelLogin(common.panelSetup):
    def GET(self):
        import urllib
        from qqwry import IPAddresss
        domain = 'ipapi.co'
        flag = os.system('ping -c 1 -w 1 %s' % domain)
        if flag == 0:
         #return public.returnJson("1",flag)
         #code = urllib.urlopen('https://'+ domain ).code
         #return public.returnJson("1",code)
         #if not code == '200': 
         result = public.httpGet('https://'+ domain +'/region_code')
         #return public.returnJson("1",result)
         if not result in  session.webname.split(','): return web.notfound();
        else:
         domains = 'ip-api.com'
         flags = os.system('ping -c 1 -w 1 %s' % domains)
         if flags == 0:
          #return public.returnJson("2",flags)
          result = public.httpGet('http://'+ domains +'/json/?lang=en')
          data = json.loads(result)
          #return public.returnJson("2",data['region'])
          if not data['region'] in  session.webname.split(','): return web.notfound();
         else:
         #https://github.com/HMBSbige/qqwry
          ips = IPAddresss("QQWry.dat")
          address = ips.getIpAddr(ips.str2ip(web.ctx.ip))
          counts = [address.count(sub) for sub in session.webname.split(',')]
          if not 1 in counts: return web.notfound();
...
class panelPublic(common.panelSetup):
    ...
    def SetSession(self):
        ...
        if not hasattr(web.ctx.session,'home'):
            web.ctx.session.home = 'https://www.baidu.com';
...
#    return web.notfound(errorStr);
    ...
    ''' % (public.getMsg('PAGE_ERR_404_TITLE'),public.getMsg('PAGE_ERR_404_TITLE'))
    return web.notfound(errorStr);
